(function () {
    ...
    style.textContent = `
        ...
        .user-ratings-header span:first-child {
            content: 'Valoraciones de Usuario'; 
        }
    ...
    async function createRatingsUI(itemId) {
        ...
        header.innerHTML = '<span>Valoraciones de Usuario</span>';
        ...
        myRatingTitle.textContent = `¿Cómo calificarías ${itemName}?`;
        ...
        ratingPrompt.textContent = 'Selecciona tu calificación';
        ...
        saveBtn.textContent = 'Publicar Valoración';
        ...
        deleteBtn.textContent = 'Borrar Valoración';
        deleteBtn.style.display = 'none';
        deleteBtn.addEventListener('click', async () => {
            if (!confirm('¿Borrar tu valoración?')) {
                return;
            }
    ...
    async function displayAllRatings(itemId, container) {
        ...
        if (totalRatings > 0) {
            avgDisplay.textContent = `★ ${averageRating.toFixed(1)} (${totalRatings} ${totalRatings === 1 ? 'valoración' : 'valoraciones'})`;
        } else {
            avgDisplay.textContent = 'Aún no hay valoraciones';
        }
        ...
        title.textContent = 'Todas las Reseñas';
        ...
        ratings.forEach(rating => {
            ...
            userName.textContent = rating.userName || rating.UserName || 'Usuario Desconocido';
    ...
    function injectRatingsUI() {
        ...
        createRatingsUI(itemId).then(ui => {
            ...
            // Function to check size and handle refresh (from model)
            const checkSizeAndRefresh = (checkName) => {
                ...
                if (hasZeroSize || hasZeroSizeFlag) {
                    ...
                    seamlessPageRefresh(itemId, isFinalCheck); // Push force flag
                }
            };

            // Multiple validation checks
            setTimeout(() => checkSizeAndRefresh('Initial'), 500);
            setTimeout(() => checkSizeAndRefresh('Subsequent'), 1500);
            setTimeout(() => checkSizeAndRefresh('Final'), 4000);

        }).catch(err => {
            console.error('[UserRatings] Error creating/injecting UI:', err);
            isInjecting = false;
        });
    }
    ...
    function handleConfigPage() {
        // This is now handled by config.js
    }

    // Monitor page changes
    window.addEventListener('hashchange', () => {
        const newUrl = window.location.href;
        console.log('[UserRatings] URL changed to:', newUrl);

        // Allow time for DOM to update
        setTimeout(injectRatingsUI, 500);
    });

    // Also monitor standard navigation (history api)
    const pushState = history.pushState;
    history.pushState = function () {
        pushState.apply(history, arguments);
        console.log('[UserRatings] pushState detected');
        setTimeout(injectRatingsUI, 500);
    };
    ...
    history.replaceState = function () {
        ...
        console.log('[UserRatings] replaceState detected');
        setTimeout(injectRatingsUI, 500);
    };

    // Initial injection
    setTimeout(injectRatingsUI, 1000); // Initial delay

    // Periodic check (fallback)
    setInterval(() => {
        if (!document.getElementById('user-ratings-ui')) {
            const urlParams = new URLSearchParams(window.location.search);
    ...
    async function displayRatingsList() {
        ...
        if (!ratingsTabContent) {
            ...
            if (!homePage) {
                console.error('[UserRatings] No se pudo encontrar la página de inicio');
                return;
            }
        ...
        // Show loading
        ratingsTabContent.innerHTML = '<div style="padding: 3em 2em; text-align: center; color: rgba(255,255,255,0.6);">Cargando valoraciones...</div>';
        ...
        try {
            ...
            if (items.length === 0) {
                ratingsTabContent.innerHTML = `
                    <div style="padding: 4em 2em; text-align: center;">
                        ...
                        <div style="font-size: 1.2em; color: rgba(255, 255, 255, 0.6);">Aún no hay elementos valorados</div>
                    </div>
                `;
                return;
            }
            ...
            const itemPromises = items.map(async (item) => {
                try {
                    ...
                    console.error('[UserRatings] Error cargando detalles del elemento:', error);
                    return null;
                }
            });
            ...
            if (itemsWithDetails.length === 0) {
                ratingsTabContent.innerHTML = `
                    <div style="padding: 4em 2em; text-align: center;">
                        <div style="font-size: 1.2em; color: rgba(255, 255, 255, 0.6);">No se pudieron cargar los detalles de los elementos</div>
                    </div>
                `;
                return;
            }
            ...
            const buildCategoryGrid = (items) => items.map(item => {
                ...
                const title = details.Name || 'Desconocido';
                ...
                sectionsHTML += `
                    <div class="verticalSection">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">Películas valoradas recientemente</h2>
                        </div>
            ...
            if (recentSeries.length > 0) {
                sectionsHTML += `
                    <div class="verticalSection">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">Series valoradas recientemente</h2>
                        </div>
                        <div is="emby-itemscontainer" class="itemsContainer padded-left padded-right vertical-wrap focuscontainer-x" data-margin="0.9%">
            ...
            if (recentEpisodes.length > 0) {
                sectionsHTML += `
                    <div class="verticalSection">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">Episodios valorados recientemente</h2>
                        </div>
                        <div is="emby-itemscontainer" class="itemsContainer padded-left padded-right vertical-wrap focuscontainer-x" data-margin="0.9%">
            ...
            const renderAllItemsSection = (page, sortBy) => {
                ...
                allItemsSection.innerHTML = `
                    <div class="verticalSection">
                        <div class="sectionTitleContainer sectionTitleContainer-cards padded-left">
                            <h2 class="sectionTitle sectionTitle-cards">Todos los elementos valorados</h2>
                        </div>
                        <div class="flex align-items-center justify-content-center flex-wrap-wrap padded-top padded-left padded-right padded-bottom focuscontainer-x" style="gap: 1em;">
                            <div class="paging">
                                <div class="listPaging">
                                    <span style="vertical-align:middle;">${startItem}-${endItem} de ${allItems.length}</span>
                            ...
                            <select is="emby-select" id="sortSelect" class="emby-select-withcolor emby-select" style="width: auto;">
                                <option value="rating-desc" ${sortBy === 'rating-desc' ? 'selected' : ''}>Nota: Alta a Baja</option>
                                <option value="rating-asc" ${sortBy === 'rating-asc' ? 'selected' : ''}>Nota: Baja a Alta</option>
                                <option value="title-asc" ${sortBy === 'title-asc' ? 'selected' : ''}>Título: A-Z</option>
                                <option value="title-desc" ${sortBy === 'title-desc' ? 'selected' : ''}>Título: Z-A</option>
                                <option value="recent" ${sortBy === 'recent' ? 'selected' : ''}>Recientemente valorados</option>
                                <option value="oldest" ${sortBy === 'oldest' ? 'selected' : ''}>Más antiguos</option>
                                <option value="count-desc" ${sortBy === 'count-desc' ? 'selected' : ''}>Más valoraciones</option>
                                <option value="count-asc" ${sortBy === 'count-asc' ? 'selected' : ''}>Menos valoraciones</option>
                            </select>
                        </div>
            ...
            console.error('[UserRatings] Error mostrando lista de valoraciones:', error);
            ratingsTabContent.innerHTML = `
    ...
    function injectRatingsTab() {
        try {
            ...
            ratingsTab.innerHTML = '<div class="emby-button-foreground">Valoraciones</div>';
            ...
            ratingsTab.addEventListener('click', async function (e) {
                ...
                try {
                    ...
                    console.error('[UserRatings] Error en displayRatingsList:', error);
                }
            });
            ...
            console.error('[UserRatings] Error inyección pestaña:', error);
        }
    }
